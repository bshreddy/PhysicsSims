...
for freq in freqs:
    each_sample_no = np.arange(duration * sps)
    waveform = np.append(waveform, np.sin(2 * np.pi * each_sample_no * freq / sps))
    each_sample_no_silent = np.zeros(int(0.001 * sps))
    waveform = np.append(waveform, np.sin(each_sample_no_silent))

waveform = waveform * 0.4
write("havana.wav", sps, np.int16(waveform * 32767))
print("saved")
